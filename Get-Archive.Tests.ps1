...
Describe 'Get-Archive' {
    ...
    It 'Хэш-суммы MD5 созданы корректно' {
		    $result.CreatedArchives.Count | Should -Be $projectsNumber
		    foreach ($i in 0..($projectsNumber-1)) {
			    $result.CreatedMD5[$i] | Should -Be "$($result.CreatedArchives[$i]).md5"
			    Get-Content $result.CreatedMD5[$i] | Should -Be (Get-FileHash $result.CreatedArchives[$i] MD5).Hash
		    }
    }
    It 'Хэш-суммы SHA1 созданы корректно' {
		    $result.CreatedArchives.Count | Should -Be $projectsNumber
		    foreach ($i in 0..($projectsNumber-1)) {
			    $result.CreatedSHA1[$i] | Should -Be "$($result.CreatedArchives[$i]).sha1"
			    Get-Content $result.CreatedSHA1[$i] | Should -Be (Get-FileHash $result.CreatedArchives[$i] SHA1).Hash
		    }
    }
    It 'Хэш-суммы SHA256 созданы корректно' {
		    $result.CreatedArchives.Count | Should -Be $projectsNumber
		    foreach ($i in 0..($projectsNumber-1)) {
			    $result.CreatedSHA256[$i] | Should -Be "$($result.CreatedArchives[$i]).sha256"
			    Get-Content $result.CreatedSHA256[$i] | Should -Be (Get-FileHash $result.CreatedArchives[$i] SHA256).Hash
		    }
    }
}
